...
public class TreeTableViewSample extends Application {
	...
	private final ImageView depIcon = new ImageView (
			new Image(getClass().getResourceAsStream("department.pnfg"))
	);

	final TreeItem<Employee> rootImapFolder =
			new TreeItem<>(new Employee("Sales Department", ""), depIcon);
	public static void main(String[] args) {
		Application.launch(TreeTableViewJava.class, args);
	}

	@Override
	public void start(Stage stage) {
		rootImapFolder.setExpanded(true);
		employees.each{employee->
			rootImapFolder.getChildren().add(new TreeItem<>(employee));
		};
		...
		TreeTableView<Employee> treeTableView = new TreeTableView<>(rootImapFolder);
